let text = `Thankyou For Using Our Website. 
...
class Letter extends React.Component { 
  ...
  _fadeIn () { 
    this.timeouts.push(setTimeout(() => { 
      ...
      this.timeouts.push(setTimeout(() => this.setState({animate: false}), 
this.animationSpeed)) 
  ...
  render () { 
    ...
    const easingFunction = `cubic-bezier(${Math.random()*.8 + +.1},0,${Math.random()*.5 + 
.25},1)` 
...
class RandomText extends React.Component { 
  ...
  componentWillReceiveProps (nextProps) { 
    if (nextProps.fontHeight !== this.props.fontHeight || nextProps.width !== 
this.props.width) { 
      ...
      if (text[i] === ' ') { 
        ...
        if (firstLetterOfWord) { 
          ...
          if (wordLength > charsPerRow) { 
            text = text.slice(0, indOfLastCharInRow - 1) + '-' + 
